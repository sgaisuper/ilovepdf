...
const os = require("os");
const { fetchBlobToFile } = require("./blob");
...
function createFileLink({ buffer, filename, contentType, tool, blobUrl }) {
  if (blobUrl) {
    ensureRoot();
    const id = newId();
    const now = Date.now();
...
async function readFileBuffer(id) {
  ...
  if (meta.blobUrl) {
    try {
      const file = await fetchBlobToFile(meta.blobUrl, meta.filename);
      if (!meta.size && file.buffer) {
