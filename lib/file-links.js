...
const TTL_MS = 2 * 60 * 60 * 1000; // 2 hours like iLovePDF
...
function isExpired(meta) {
  return !meta || !meta.expiresAt || Date.now() > meta.expiresAt || meta.revoked;
}
...
function getFileLink(id) {
  ...
  if (!meta) return null;
  if (isExpired(meta)) return { ...meta, expired: true };
  return { ...meta, expired: false };
}

function recordDownload(id, info = {}) {
  const meta = readMeta(id);
  if (!meta || isExpired(meta)) return null;
  const now = Date.now();
  meta.downloads += 1;
