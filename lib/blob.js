...
function usePresignedUploads() {
  // Prefer classic client uploads via BLOB_READ_WRITE_TOKEN when available.
  if (process.env.BLOB_READ_WRITE_TOKEN) return false;
  return !!(process.env.BLOB_WEBHOOK_PUBLIC_KEY && process.env.BLOB_STORE_ID);
}
...
async function fetchBlobToFile(url, filename) {
  ...
  try {
    const getOpts = { access: "private", useCache: false };
    if (process.env.BLOB_READ_WRITE_TOKEN) {
      getOpts.token = process.env.BLOB_READ_WRITE_TOKEN;
    }
    const result = await get(url, getOpts);
  ...
  try {
    ...
    if (rwToken) {
      ...
      res.end(
        JSON.stringify({
          error:
            "Blob client uploads require BLOB_READ_WRITE_TOKEN (preferred) or BLOB_WEBHOOK_PUBLIC_KEY + BLOB_STORE_ID",
        })
      );
      return;
    }
...
async function putBufferToBlob(pathname, buffer, contentType) {
  ...
  return put(pathname, buffer, opts);
}
