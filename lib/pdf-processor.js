...
async function splitPdf(files, options = {}) {
  const file = files[0];
  const src = await loadPdf(file.buffer);
  const mode = options.mode || "pages"; // pages | range
  const zip = new JSZip();
...
function parseRanges(text, pageCount) {
  // "1-3,5,7-9" -> arrays of 0-based indexes
...
async function pdfToJpg(files) {
  ...
  return withTempDir(async (dir) => {
    ...
    await run("pdftoppm", ["-jpeg", "-r", "150", input, prefix]);
    const pages = fs
      .readdirSync(dir)
    ...
    if (pages.length === 1) {
      return {
        buffer: fs.readFileSync(path.join(dir, pages[0])),
        filename: "page-1.jpg",
        contentType: "image/jpeg",
      };
    }
    const zip = new JSZip();
    pages.forEach((f, i) => zip.file(`page-${i + 1}.jpg`, fs.readFileSync(path.join(dir, f))));
    return {
    ...
    for (const file of files) {
      ...
      if (!converted) {
        const expected = path.join(
          dir,
          path.basename(inName, path.extname(inName)) + `.${outExt}`
        );
        if (fs.existsSync(expected)) {
          results.push({
            name: path.basename(expected),
            buffer: fs.readFileSync(expected),
          });
        } else {
          throw new Error(`Conversion to ${outExt} failed for ${file.filename}`);
        }
      } else {
        results.push({
          name: converted,
...
async function pdfToPowerpoint(files) {
  try {
    ...
    const results = [];
    for (const file of files) {
      const pages = await rasterizePdf(file.buffer);
...
async function rasterizePdf(buffer) {
  return withTempDir(async (dir) => {
    ...
    fs.writeFileSync(input, buffer);
    const prefix = path.join(dir, "page");
    await run("pdftoppm", ["-jpeg", "-r", "120", input, prefix]);
    return fs
      .readdirSync(dir)
      .filter((f) => f.startsWith("page") && f.endsWith(".jpg"))
      .sort()
...
async function pdfToMarkdown(files) {
  return withTempDir(async (dir) => {
    ...
    fs.writeFileSync(input, file.buffer);
    const { stdout } = await run("pdftotext", ["-layout", input, "-"]);
    const text = stdout.toString("utf8").trim() || "(No extractable text)";
...
async function summarizePdf(files) {
  return withTempDir(async (dir) => {
    ...
    fs.writeFileSync(input, file.buffer);
    const { stdout } = await run("pdftotext", ["-layout", input, "-"]);
    const text = stdout.toString("utf8").replace(/\s+/g, " ").trim();
...
async function translatePdf(files, options = {}) {
  // Lightweight demo translation: wrap extracted text. Real MT would need an API key.
  return withTempDir(async (dir) => {
    const file = files[0];
    const input = path.join(dir, "input.pdf");
    fs.writeFileSync(input, file.buffer);
...
async function comparePdf(files) {
  ...
  return withTempDir(async (dir) => {
    ...
    fs.writeFileSync(a, files[0].buffer);
    fs.writeFileSync(b, files[1].buffer);
    const textA = (await run("pdftotext", ["-layout", a, "-"])).stdout.toString("utf8");
    const textB = (await run("pdftotext", ["-layout", b, "-"])).stdout.toString("utf8");
...
async function redactPdf(files, options = {}) {
  ...
  // Text-layer redaction approximation: overlay black boxes is hard without positions.
  // Practical approach: extract text, replace terms, rewrite a text PDF.
  return withTempDir(async (dir) => {
    const file = files[0];
    const input = path.join(dir, "input.pdf");
    fs.writeFileSync(input, file.buffer);
    let text = (await run("pdftotext", ["-layout", input, "-"])).stdout.toString("utf8");
...
async function ocrPdf(files) {
  // Without tesseract OCR engine, produce searchable text PDF from existing text layer / pdftotext.
  // If tesseract exists, use it on rasterized pages.
  const hasTesseract = fs.existsSync("/usr/bin/tesseract");
  const file = files[0];
  return withTempDir(async (dir) => {
    const input = path.join(dir, "input.pdf");
    fs.writeFileSync(input, file.buffer);
    if (!hasTesseract) {
      // fallback: rewrite extractable text into a clean searchable PDF
      const text = (await run("pdftotext", ["-layout", input, "-"])).stdout.toString("utf8");
      if (!text.trim()) {
        throw new Error(
          "OCR requires Tesseract for scanned PDFs. Install tesseract-ocr, or upload a text PDF."
        );
      }
      const doc = await PDFDocument.create();
      const font = await doc.embedFont(StandardFonts.Helvetica);
    ...
    for (const imgName of images) {
      ...
      // invisible-ish text layer substitute: tiny footer note
...
async function editPdfPlaceholder(files, options = {}) {
  // Minimal "edit": stamp user text onto first page
...
async function pdfFormsPassthrough(files) {
  // Preserve form-capable PDF; ensure saved with pdf-lib
  const file = files[0];
  const src = await loadPdf(file.buffer);
...
const TOOL_HANDLERS = {
  ...
  "/word_to_pdf": (files) => libreOfficeConvert(files, "pdf"),
  "/powerpoint_to_pdf": (files) => libreOfficeConvert(files, "pdf"),
  "/excel_to_pdf": (files) => libreOfficeConvert(files, "pdf"),
